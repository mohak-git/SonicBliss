...
document.addEventListener('DOMContentLoaded', () => {
    ...
    authBtn.addEventListener('click', async () => {
        ...
        clientId = '59b5771f346c465197f26366abc1cacf';
        clientSecret = 'da9b81955d84480993ec3494b3e31cd3';
    ...
    let playSong = async function () {
        ...
        async function getPoster() {
            ...
            const posterOptions = {
                headers: {
                    'x-rapidapi-key': 'ef8e72a838msh1e5274c1bc90f89p141f63jsn1bab8d311445'
                }
            };
            ...
            try {
                ...
                posterImage = "https://plus.unsplash.com/premium_photo-1682125488670-29e72e5a7672?q=80&w=1888&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D";
            }
        }
        await getPoster();
        ...
        Math.floor(totalSec / 10) == 0 ? endTime.innerText = (`${totalMin}:0${totalSec}`) : endTime.innerText = (`${totalMin}:${totalSec}`);

        startTime.innerText = '0:00';
        let progressTimer = 1;
        function progressBarMovement() {
            if ((progressTimer === 1) || playBtn.disabled && progressTimer < durationInMin) {
                let percent = (progressTimer / durationInMin) * 100;
                progressBar.style.setProperty('--percent', `${percent}%`);

                let secVar = (Math.floor((progressTimer % 60) / 10) != 0) ? Math.floor(progressTimer % 60) : '0' + Math.floor(progressTimer % 60);
